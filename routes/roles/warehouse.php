...
Route::middleware('role:'.User::ROLE_WAREHOUSE)->prefix('warehouse')->name('warehouse.')->group(function (): void {
    ...
    Route::get('orders/reconciliation-bulk/meta', [ReconciliationBulkController::class, 'meta'])->name('orders.reconciliation-bulk.meta');
    Route::post('orders/reconciliation-bulk/inspect', [ReconciliationBulkController::class, 'inspect'])->name('orders.reconciliation-bulk.inspect');
    Route::post('orders/reconciliation-bulk/update', [ReconciliationBulkController::class, 'updateByCodes'])->name('orders.reconciliation-bulk.update');
    Route::get('orders/reconciliation-bulk/template', [ReconciliationBulkController::class, 'template'])->name('orders.reconciliation-bulk.template');
    Route::post('orders/reconciliation-bulk/upload', [ReconciliationBulkController::class, 'upload'])->name('orders.reconciliation-bulk.upload');
    Route::get('orders/reconciliation-bulk/history', [ReconciliationBulkController::class, 'history'])->name('orders.reconciliation-bulk.history');
    Route::post('orders/reconciliation-bulk/batches/{batch}/apply', [ReconciliationBulkController::class, 'apply'])->whereNumber('batch')->name('orders.reconciliation-bulk.apply');
    Route::post('orders/reconciliation-bulk/batches/{batch}/clear', [ReconciliationBulkController::class, 'clear'])->whereNumber('batch')->name('orders.reconciliation-bulk.clear');
