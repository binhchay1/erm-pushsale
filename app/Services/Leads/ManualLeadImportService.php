...
class ManualLeadImportService
{
    ...
    protected function ingestOne(array $data, ?User $forceSale = null): LeadIngestion
    {
        $payload = [
            ...
            'message' => $this->str($data['message'] ?? $data['note'] ?? null),
            'shipping_address' => $this->str($data['shipping_address'] ?? $data['address'] ?? null),
            'shipping_notes' => $this->str($data['shipping_notes'] ?? null),
            'deposit' => (int) MoneyParser::parse($data['deposit'] ?? 0),
            'shipping_fee_collected' => (int) MoneyParser::parse($data['shipping_fee_collected'] ?? 0),
    ...
    protected function resolveItemsFromForm(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $items[] = [
                ...
                'discount_amount' => max(0, (int) ($row['discount_amount'] ?? 0)),
