...
class SalesLeaderboardService
{
    ...
    public function build(MarketingRankingFilterData $filter): array
    {
        ...
        $ingestions = LeadContactMetrics::applyCountableScope(LeadIngestion::query())
            ->with([
                'order:id,sale_user_id,is_returning_customer',
                'relatedOrder:id,sale_user_id,is_returning_customer',
            ])
            ->whereBetween('lead_ingestions.created_at', [$filter->dateFrom, $filter->dateTo])
            ->where(function (Builder $query) use ($userIds): void {
                $query->whereHas('order', fn (Builder $order) => $order->whereIn('sale_user_id', $userIds))
                    ->orWhereHas('relatedOrder', fn (Builder $order) => $order->whereIn('sale_user_id', $userIds));
            })
            ->get();
        ...
        $rows = $users->map(function (User $user) use ($ingestions, $orders, $filter): array {
            $contacts = $ingestions->filter(function (LeadIngestion $lead) use ($user): bool {
                $order = $lead->order ?? $lead->relatedOrder;

                return (int) ($order?->sale_user_id ?? 0) === (int) $user->id;
            });
            $userOrders = $orders->where('sale_user_id', $user->id);

            $newContacts = $contacts->filter(fn (LeadIngestion $lead): bool => ! (bool) ($lead->order ?? $lead->relatedOrder)?->is_returning_customer)->count();
            $oldContacts = $contacts->filter(fn (LeadIngestion $lead): bool => (bool) ($lead->order ?? $lead->relatedOrder)?->is_returning_customer)->count();
