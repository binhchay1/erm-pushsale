...
class ManualLeadDriver implements LeadPayloadNormalizer
{
    ...
    public function normalize(array $payload): array
    {
        ...
        return [
            ...
            'message' => Arr::get($payload, 'message') ?? Arr::get($payload, 'note'),
            'quantity' => $quantity,
            'shipping_address' => Arr::get($payload, 'shipping_address') ?? Arr::get($payload, 'address'),
            'shipping_notes' => Arr::get($payload, 'shipping_notes'),
            'deposit' => max(0, (int) (Arr::get($payload, 'deposit') ?? 0)),
            'shipping_fee_collected' => max(0, (int) (Arr::get($payload, 'shipping_fee_collected') ?? 0)),
            'discount' => max(0, (int) (Arr::get($payload, 'discount') ?? 0)),
