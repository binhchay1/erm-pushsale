 ...
 *
 * DOM giống hệt mẫu Pushsale để CSS dùng chung được cho cả trang React lẫn
 * trang template inject HTML:
 *
 *   .m-header-wrap > .m-header  (title | filter chính | nút)
 *   .m-header-wrap > .ps-page-header__advanced  (filter nâng cao, ngoài .m-header)
 *
...
function PageHeaderBar({ title, subtitle, icon: Icon, filters, actions, advanced, pageCode, className, defaultCollapsed, collapsible, sticky }) {
    ...
    return (
        ...
        >
            ...
            {hasAdvanced ? (
                <div className="ps-page-header__advanced box-body" hidden={collapsed || undefined}>
                    {advanced}
                </div>
            ) : null}
        </div>
