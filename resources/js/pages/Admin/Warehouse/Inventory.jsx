...
export default function Inventory({ report, filterOptions = {}, intakeUrl, exportUrl, approverOptions = [] }) {
    ...
    const [filters, setFilters] = useState({
        search: f.search ?? '',
        warehouse_id: String(f.warehouse_id ?? ''),
    ...
    const submitMovement = (event) => {
        event.preventDefault();
        movement.post(mode === 'intake' ? intakeUrl : exportUrl, {
            preserveScroll: true,
    ...
    const exportCsv = () => {
        ...
        const lines = rows.map((row) => [
            row.id, row.warehouseName, row.productName, row.sku, row.uom, row.batchCode,
            row.expiryDate, row.locationCode, row.stockQuantity, row.pendingSalesQuantity,
        ...
        anchor.href = url;
        anchor.download = 'danh-sach-san-pham-kho.csv';
        anchor.click();
    ...
    return (
        <AppLayout>
            ...
            <PushsalePageShell
                ...
                advancedFilters={(
                    <form className="ps-filter-row ps-inventory-filters" onSubmit={search}>
                        <select className="form-control" value={filters.warehouse_id} onChange={(event) => setFilters((old) => ({ ...old, warehouse_id: event.target.value }))}>
                            <option value="">--Chọn kho--</option>
                            {(filterOptions.warehouses ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
                        </select>
                        <select className="form-control" value={filters.product_id} onChange={(event) => setFilters((old) => ({ ...old, product_id: event.target.value }))}>
                            <option value="">--Chọn sản phẩm--</option>
                            {(filterOptions.products ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}{item.sku ? ` (${item.sku})` : ''}</option>)}
                        </select>
                        ...
                        <select className="form-control" value={filters.business_status} onChange={(event) => setFilters((old) => ({ ...old, business_status: event.target.value }))}>
                            <option value="">--Trạng thái kinh doanh--</option>
                            <option value="active">Đang kinh doanh</option>
                            <option value="stopped">Ngừng kinh doanh</option>
                        </select>
                        <button className="btn btn-sm btn-default" type="submit"><i className="fa fa-filter" /> Lọc</button>
                    </form>
                )}
                toolbar={(
                    <div className="ps-toolbar ps-inventory-toolbar">
                        <button type="button" className="btn btn-sm btn-primary" onClick={openVoucherEntry}>
                            <i className="fa fa-exchange" /> Xuất / nhập kho
                        </button>
                        <button type="button" className="btn btn-sm btn-default" onClick={() => openMovement('intake')}>
                            <i className="fa fa-map-marker" /> Cập nhật vị trí
                        </button>
            ...
            >
                <div className="ps-table-scroll ps-inventory-table-wrap">
                    <table className="table table-bordered ps-source-table ps-inventory-table">
                        <thead>
                            <tr>
                                ...
                                <th>Vị trí</th>
                                <th>Tồn kho</th>
                                <th>Chờ xuất bán hàng</th>
                                <th>SL sắp hết hàng</th>
                                <th>Ngừng KD</th>
                                <th>Cập nhật</th>
                                <th />
                            </tr>
                        ...
                        <tbody>
                            {rows.length ? rows.map((row) => (
                                <tr key={row.id}>
                                    ...
                                    <td><strong>{row.productName}</strong>{row.sku && <small> ({row.sku})</small>}</td>
                                ...
                                <tr><td colSpan="14" className="ps-empty">Chưa có dữ liệu tồn kho.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            <DialogShell open={movementOpen} onClose={() => setMovementOpen(false)}>
                <form onSubmit={submitMovement} className="ps-form-grid ps-form-grid-2">
                    <div className="span-2 ps-tabs">
                        <button type="button" className={mode === 'intake' ? 'active' : ''} onClick={() => setMode('intake')}>Nhập kho</button>
                        <button type="button" className={mode === 'export' ? 'active' : ''} onClick={() => setMode('export')}>Xuất kho</button>
                    </div>
